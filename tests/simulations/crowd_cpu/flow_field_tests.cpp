...
TEST(CrowdCpuFlowField, FlowVectorsRouteAroundBlockedCells) {
    SimulationConfig config = baseConfig();
    Simulation sim{config};
    ...
    ASSERT_LT(blocked, flow.size());
    EXPECT_LE(flow[blocked].length(), 1e-4f);

    const std::size_t neighbor = cellIndex(config, Vec2{30.0f, 50.0f});
    ASSERT_LT(neighbor, flow.size());
    EXPECT_GT(flow[neighbor].length(), 0.1f);
}
