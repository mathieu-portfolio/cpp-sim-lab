...
using namespace bubbles_cpu_gpu;
...
int main() {
    ...
    for (std::size_t entityCount : entityCounts) {
        ...
        for (std::size_t modeIndex = 0; modeIndex < modes.size(); ++modeIndex) {
            ...
            progress.advance();
            std::cout << "bubbles_cpu_gpu," << entityCount << "," << mode.backend << "," << mode.threading << "," << MeasuredFrames << "," << result.totalMs << "," << result.avgFrameMs << "," << relative << "," << result.bubbleCount << "," << result.interactionChecks << "," << result.collisionsResolved << "\n";
        }
    }
