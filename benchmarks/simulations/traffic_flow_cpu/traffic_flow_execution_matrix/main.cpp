...
int main() {
    ...
    for (std::size_t entityCount : entityCounts) {
        ...
        for (std::size_t modeIndex = 0; modeIndex < modes.size(); ++modeIndex) {
            ...
            progress.advance();
            std::cout << "traffic_flow_cpu," << entityCount << "," << mode.backend << "," << mode.threading << "," << result.frames << "," << result.totalMs << "," << result.avgFrameMs << "," << relative << "," << result.throughputPerSecond << "," << result.averageSpeed << "," << result.averageQueueLength << "\n";
        }
    }
