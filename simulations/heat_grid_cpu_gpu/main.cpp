...
namespace {
using heat_grid_cpu_gpu::BoundaryMode;
...
int main() {
    heat_grid_cpu_gpu::SimulationConfig config;
    ...
    InitWindow(static_cast<int>(config.width), static_cast<int>(config.height), "heat_grid_cpu_gpu");
    SetTargetFPS(60);

    heat_grid_cpu_gpu::Simulation sim{config};
    sim.reset();
    ...
    while (!WindowShouldClose()) {
        ...
        constexpr std::size_t paramCount =
            std::tuple_size_v<decltype(simfw::ui::ConfigUiTraits<heat_grid_cpu_gpu::SimulationConfig>::fields)>;
        ...
        if (controls.uiMode != simfw::ui::UiMode::None) {
            ...
            cursor.draw("heat_grid_cpu_gpu", 20, BLACK);
