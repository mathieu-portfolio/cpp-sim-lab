...
Vec2 agentPosition(const Agent &agent) { return agent.position; }
...
int main() {
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
      ...
      if (isSelectionModifierDown()) {
        selectedAgent = simfw::ui::findClosestEntity(
            sim.getEntities(), mouseWorld, SelectionPickRadius / camera.zoom,
            agentPosition);
...

    {
      const Vec2 mouseWorld =
          simfw::ui::screenToWorld(GetMousePosition(), camera);
      const auto paintMode =
          (IsKeyDown(KEY_E) ? simfw::simulation::ObstaclePaintMode::Erase
                            : simfw::simulation::ObstaclePaintMode::Block);
      obstacleBrush.paint(sim.obstacleMask(),
                          IsMouseButtonDown(MOUSE_RIGHT_BUTTON), mouseWorld,
                          ObstacleBrushRadius / camera.zoom, paintMode);
    }
    ...
    if (IsKeyPressed(KEY_L)) {
      simfw::ui::loadObstacleMaskFromPng(ScenarioObstacleMapPath,
                                         sim.obstacleMask());
    }
    if (IsKeyPressed(KEY_O)) {
      simfw::ui::exportObstacleMaskToPng(ScenarioObstacleMapPath,
                                         sim.obstacleMask());
    }
    ...
    if (controls.showDebug) {
      ...
      const std::size_t gridWidth =
          static_cast<std::size_t>(config.width / config.gridCellSize) + 1;
      const std::size_t gridHeight =
          static_cast<std::size_t>(config.height / config.gridCellSize) + 1;
      const int goalCellX =
          std::clamp(static_cast<int>(sim.getGoal().x / config.gridCellSize), 0,
                     static_cast<int>(gridWidth - 1));
      const int goalCellY =
          std::clamp(static_cast<int>(sim.getGoal().y / config.gridCellSize), 0,
      ...
      for (std::size_t y = 0; y < gridHeight; ++y) {
        for (std::size_t x = 0; x < gridWidth; ++x) {
          ...
          const Vec2 cellMin{static_cast<float>(x) * config.gridCellSize,
          ...
          if (cellIndex < cost.size() && cost[cellIndex] >= 255.0f) {
            DrawRectangleV(simfw::ui::toRaylib(cellMin),
                           Vector2{config.gridCellSize, config.gridCellSize},
                           Fade(DARKGRAY, 0.5f));
          }

          if (static_cast<int>(x) == goalCellX &&
              static_cast<int>(y) == goalCellY) {
            DrawRectangleLinesEx(Rectangle{cellMin.x, cellMin.y,
                                           config.gridCellSize,
                                           config.gridCellSize},
                                 2.0f, GREEN);
          }
          ...
          if (direction.lengthSquared() > 0.000001f) {
            const Vec2 debugDir =
                direction.normalized() * (config.gridCellSize * 0.45f);
            DrawLineV(simfw::ui::toRaylib(cellCenter),
                      simfw::ui::toRaylib(cellCenter + debugDir), SKYBLUE);
          }

          if (showIntegrationValues && cellIndex < integration.size() &&
              std::isfinite(integration[cellIndex])) {
            DrawText(TextFormat("%.0f", integration[cellIndex]),
                     static_cast<int>(cellMin.x + 2.0f),
                     static_cast<int>(cellMin.y + 2.0f), 10, GRAY);
          }
        }
      }
    }

    DrawCircleLines(static_cast<int>(sim.getGoal().x),
                    static_cast<int>(sim.getGoal().y), config.goalRadius,
                    GREEN);

    if (selectedAgent) {
      simfw::ui::drawSelectionRing(sim.getEntities()[*selectedAgent].position,
    ...
    if (controls.uiMode != simfw::ui::UiMode::None) {
      ...
      if (selectedAgent) {
        ...
        cursor.draw(
            TextFormat("Selected agent: %d", static_cast<int>(*selectedAgent)),
            16, YELLOW);
        cursor.draw(
            TextFormat("pos: %.1f, %.1f", agent.position.x, agent.position.y));
        cursor.draw(
            TextFormat("vel: %.1f, %.1f", agent.velocity.x, agent.velocity.y));
      }
