...
int main() {
    heat_grid::SimulationConfig config;
    ...
    InitWindow(static_cast<int>(config.width), static_cast<int>(config.height), "heat_grid");
    SetTargetFPS(60);

    heat_grid::Simulation sim{config};
    sim.reset();
    ...
    while (!WindowShouldClose()) {
        ...
        if (controls.uiMode != simfw::ui::UiMode::None) {
            ...
            cursor.draw("heat_grid", 20, BLACK);
